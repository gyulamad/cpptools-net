...
public:
  ...
  ZzzProxy()
      : WolProxy(), lastClientCount(0), serverState(SERVER_UNKNOWN),
  ...
  void forward(int port, const string &host, int hport,
    ...
    this->idleTimeoutSec =
        (idleSeconds > 0) ? idleSeconds : 60; // minimum 60 sec
    ...
    LOG(string("Idle timeout: ") + to_string(this->idleTimeoutSec) +
        " seconds");
...
private:
  inline static time_t getCurrentTimeSec() { return default_get_time_sec(); }
...
public:
  ...
  void triggerWakeup() {
    ...
    try {
      ...
      // Add delay so we don't repeatedly issue commands - skip next few check
      // cycles
      ...
      LOG_ERROR(string("[WAKE] Failed executing command! Exception caught: ") +
                e.what());
      serverState = (prevState == SERVER_UNKNOWN
                         ? SERVER_DOWN
                         : prevState); // Revert or stay DOWN depending context.
    }

    wakeupInProgress = false;
  }

  /// Triggered when backend disconnects unexpectedly — update internal
  /// availability status. Note that base class doesn't expose explicit callback
  /// hook here but can infer from client-side events indirectly through other
  /// means later as needed.
  // void onBackendDisconnect(int fd){...}

  /**
   * Called each event loop iteration (every ~100 ms).
   *
   * Monitors connection count vs thresholds; triggers appropriate actions based
   * on elapsed idleness durations beyond configured limits.
   */
  void onTick() override {
    constexpr int TICKS_PER_CHECK =
        10; // Check once/sec instead of waiting full cycle
    ...
    if (tickCounter >= TICKS_PER_CHECK) {
      ...
      case SERVER_UNKNOWN: {
        ...
        // If no clients yet, assume down initially until proven otherwise upon
      ...
      case SERVER_DOWN: {
        // When a fresh client appears while we're marked offline AND not
        // already waking up,
      ...
      case SERVER_UP: {
        ...
        const string idleInfo = "Idle " + to_string(idleDurationSeconds) +
                                "s / timeout " + to_string(idleTimeoutSec) +
                                "s";

        // Only consider sleeping when ALL clients have disconnected completely:
        if (!hasClients && idleDurationSeconds >= idleTimeoutSec &&
            !sleepCmd.empty()) {

          triggerSleep();

        } else if (!hasClients) {
          // Convert enum value into human-readable label manually via
          // switch-case or simple mapping approach:
        ...
        // While transitioning between states just wait for operations in
        // progress...
...
private:
  void triggerSleep() {
    ...
    try {
      ...
      LOG_ERROR(string("[SLEEP] Failed executing command! Exception caught: ") +
                e.what());
